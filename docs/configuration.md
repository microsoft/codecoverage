...
- [Settings under CodeCoverage tag](#settings-under-codecoverage-tag)
  - [Merging with the built-in default lists](#merging-with-the-built-in-default-lists)
- [Example](#example)
...
| ModulePaths | List of strings | — | — | Include and exclude lists for module paths.<br>Empty "Include" clauses imply all; empty "Exclude" clauses imply none. Each element in the list is a regular expression (ECMAScript syntax). See /visualstudio/ide/using-regular-expressions-in-visual-studio. An item must first match at least one entry in the include list to be included. Included items must then not match any entries in the exclude list to remain included.<br>Supports the optional `mergeDefaults` attribute (default `True`); set to `False` to ignore the built-in default exclusions instead of merging with them. See [Merging with the built-in default lists](#merging-with-the-built-in-default-lists). | `<ModulePaths><Exclude><ModulePath>.*CPPUnitTestFramework.*</ModulePath></Exclude></ModulePaths>` | `"ModulePaths": {"Exclude": [".*CPPUnitTestFramework.*"]}` | 17.14 |
| PublicKeyTokens | List | — | — | Include and exclude lists for public key tokens.<br>Matches the public key token of a signed assembly.<br>Supports the optional `mergeDefaults` attribute (default `True`); set to `False` to ignore the built-in default exclusions instead of merging with them. See [Merging with the built-in default lists](#merging-with-the-built-in-default-lists). | `<PublicKeyTokens><Exclude><PublicKeyToken>^B77A5C561934E089$</PublicKeyToken></Exclude></PublicKeyTokens>` | `"PublicKeyTokens": {"Exclude": ["^B77A5C561934E089$"]}` | 17.14 |
| CompanyNames | List of strings | — | — | Include and exclude lists for company names.<br>Matches the company name property in the assembly.<br>Supports the optional `mergeDefaults` attribute (default `True`); set to `False` to ignore the built-in default exclusions instead of merging with them. See [Merging with the built-in default lists](#merging-with-the-built-in-default-lists). | `<CompanyNames><Exclude><CompanyName>.*microsoft.*</CompanyName></Exclude></CompanyNames>` | `"CompanyNames": {"Exclude": [".*microsoft.*"]}` | 17.14 |
| Attributes | List of strings | — | — | Include and exclude lists for attributes.<br>Matches attributes on any code element.<br>Supports the optional `mergeDefaults` attribute (default `True`); set to `False` to ignore the built-in default exclusions instead of merging with them. See [Merging with the built-in default lists](#merging-with-the-built-in-default-lists). | `<Attributes><Exclude><Attribute>^System\.Diagnostics\.DebuggerHiddenAttribute$</Attribute></Exclude></Attributes>` | `"Attributes": {"Exclude": ["^System\\.Diagnostics\\.DebuggerHiddenAttribute$"]}` | 17.14 |
| Sources | List of strings | — | — | Include and exclude lists for source files.<br>Matches the path of the source files in which each method is defined.<br>Supports the optional `mergeDefaults` attribute (default `True`); set to `False` to ignore the built-in default exclusions instead of merging with them. See [Merging with the built-in default lists](#merging-with-the-built-in-default-lists). | `<Sources><Exclude><Source>.*\\atlmfc\\.*</Source></Exclude></Sources>` | `"Sources": {"Exclude": [".*\\\\atlmfc\\\\.*"]}` | 17.14 |
| Functions | List of strings | — | — | Include and exclude lists for functions.<br>Matches fully qualified names of functions. Use `\.` to delimit namespaces in C# or Visual Basic, `::` in C++.).<br>Supports the optional `mergeDefaults` attribute (default `True`); set to `False` to ignore the built-in default exclusions instead of merging with them. See [Merging with the built-in default lists](#merging-with-the-built-in-default-lists). | `<Functions><Exclude><Function>^Fabrikam\.UnitTest\..*</Function></Exclude></Functions>` | `"Functions": {"Exclude": ["^Fabrikam\\.UnitTest\\..*"]}` | 17.14 |
| **Symbols** | | | | | | | |
...
| **Instrumentation** | | | | | | | |
| UseVerifiableInstrumentation | `True`, `False` | `True` | `False` | Emit verifiable probes instead of direct memory writes. Enable this on .NET Framework when instrumenting code that must pass Code Access Security verification (for example functions marked `SecuritySafeCritical`/`SecurityCritical`), or when tests throw `System.Security.VerificationException` while coverage is enabled. Coverage inserts a probe that redirects to a helper function which performs the write, keeping the instrumented method verifiable. See [Verifiable instrumentation (probes)](instrumentation.md#verifiable-instrumentation-probes). | `<UseVerifiableInstrumentation>True</UseVerifiableInstrumentation>` | `"UseVerifiableInstrumentation": true` | 17.14 |
...
Six of the settings above are include/exclude lists: `ModulePaths`, `PublicKeyTokens`,
`CompanyNames`, `Attributes`, `Sources`, and `Functions`. Code coverage ships with a set of
built-in default exclusions for these lists (for example, common test frameworks and
attributes such as `System.Diagnostics.DebuggerNonUserCodeAttribute` and
`System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverageAttribute`). Each list supports an
optional `mergeDefaults` attribute that controls how your entries combine with those
built-in defaults. Note that the attribute name is camelCase (`mergeDefaults`) and is
case-sensitive, even though the surrounding list keys (`Attributes`, `Exclude`, and so on)
are PascalCase.

| `mergeDefaults` | Behavior |
| --- | --- |
| `True` (default) | Your entries are **added** to the built-in default exclusions. You cannot remove or override a built-in entry, so anything the defaults exclude stays excluded. |
| `False` | The built-in defaults are **ignored**; only the entries you provide are used. |

Because the default is `True`, supplying an empty `<Exclude />` does **not** re-enable
coverage for code the defaults already exclude. For example, classes annotated with
`[DebuggerNonUserCode]` stay excluded because the built-in
`System.Diagnostics.DebuggerNonUserCodeAttribute` exclusion is merged back in. To measure
such code, set `mergeDefaults="False"` on the list so the built-in exclusions are not
applied:

```xml
<Configuration>
  <CodeCoverage>
    <Attributes mergeDefaults="False">
      <Exclude />
    </Attributes>
  </CodeCoverage>
</Configuration>
...
The equivalent JSON uses the camelCase key `"mergeDefaults": false`:
...
> **Note:** Setting `mergeDefaults="False"` disables **all** built-in exclusions for that
> list, including useful ones such as
> `System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverageAttribute`. If you still want any
> of those defaults, re-add them explicitly in your `Exclude` list.
